<template>
	<view>
		<uni-forms ref="employeeRef" :modelValue="employeeForm" :rules="employeeFormRules" label-width="90px">

			<uni-forms-item label="用户名" required name="userName">
				<uni-easyinput v-model="employeeForm.userName" placeholder="请输入员工名" />
			</uni-forms-item>
			<uni-forms-item label="手机号" name="phone">
				<u-input maxlength="11" type="number" placeholder="请输入手机号" border="surround" v-model="employeeForm.phone"
					clearable shape="circle" class="inputStyle">

				</u-input>
			</uni-forms-item>
      <uni-forms-item label="角色">
        <uni-data-checkbox v-model="employeeForm.role" :localdata="roleItems"></uni-data-checkbox>
      </uni-forms-item>
						...
						rules: [{
							...
							},
							{
								validateFunction: (rule, value,data,callback)=> {									
									let obj = this.hotel.employee.find(item=>{ return item.phone==value});
									if(obj){
										callback('已存在相同手机号');
									}
